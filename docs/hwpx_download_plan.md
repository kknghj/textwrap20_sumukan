...
> **상태:** MVP 구현 완료 (단일 페이지 580칸) · **대각선 템플릿 기준 다중 페이지 PoC 구현 및 수동 열기 검증 완료** (단일 `hp:tbl` `rowCnt`·`cellAddr` 행 확장)  
> **템플릿:** `assets/templates/practice_diagonal_20x29/` (MVP·다중 페이지 PoC), 후속: 방안선·십자선 (별도 fixture 검증 전까지 범위 밖)  
> **최종 수정:** 2026-06-16
...
| **다중 페이지** | **대각선 템플릿 기준 PoC 구현 및 수동 열기 검증 완료** — 단일 `hp:tbl`의 `rowCnt` 및 `cellAddr` row 확장 (`rowCnt = 29 × 페이지수`). 표 문단 복제·page break 삽입 방식은 사용하지 않음. 기능명은 **실험적 다중 페이지 PoC** 유지. |
...
### 후속 버전 (2단계) — PoC 구현·검증 완료 (대각선 템플릿)

- **구현 방식:** 단일 `hp:tbl`의 `rowCnt`를 `29 × 페이지수`로 확장하고, `cellAddr`의 `rowAddr`를 연속 증가시키며 29행 블록을 append.
- **사용하지 않는 방식:** 표 문단(`hp:p`) 복제, 별도 `hp:tbl` 복제, page break XML 삽입 (한글 수동 fixture에 해당 구조 없음 — §O 참고).
- 하나의 `.hwpx`에 여러 20×29 연습장 페이지 포함.
- **범위:** `practice_diagonal_20x29`만 적용. 방안선·십자선은 별도 fixture 검증 전까지 범위 밖.

### 구현 판단 (2026-06-16)

**초기 B안(표 문단 복제 + page break)은 채택하지 않음.** `fixtures/hwpx_2page_diagonal_manual` 분석 및 PoC 구현·한글 수동 열기 검증으로 **단일 표 행 확장** 방식이 확정됨. §M·§N·§O 참고.
...
| **7** | 다중 페이지 지원 (대각선, 실험적 PoC) | 단일 `hp:tbl` `rowCnt`·`cellAddr` 행 확장, 580칸 초과 허용 — **구현·수동 열기 검증 완료** (§F·§O 참고) |
...
4. section0.xml 파싱 → 기존 단일 hp:tbl 확보
5. 페이지 수만큼 29행 블록을 복제·append (extendPracticeTableRows)
6. hp:tbl@rowCnt = 29 × 페이지수, cellAddr.rowAddr 오프셋 적용
7. 전체 셀에 페이지 순서대로 fillPracticeCells
8. XMLSerializer → buildHwpx → Blob 다운로드
```

**채택하지 않은 초기안:** 표 문단(`hp:p`) deep clone, 별도 `hp:tbl` 복제, page break XML 삽입.
...
| `buildMultiPageSection.ts` | 29행 블록 복제·append, `rowCnt`·`cellAddr` 갱신, 셀 채우기 |
| `parseSection.ts` (확장) | 단일 `hp:tbl` 파싱·검증 (`rowCnt`·`hp:tc` 개수 확인) |
...
| --- | --- |
| 580칸 | 1페이지, `hp:tbl` 1개 |
| 581칸 | 2페이지, `hp:tbl` 1개, `rowCnt=58` |
| 1160칸 | 2페이지, `hp:tbl` 1개, `rowCnt=58` |
| 1161칸 | 3페이지, `hp:tbl` 1개, `rowCnt=87` |
| 페이지 1 첫 칸 | `chars[0]` |
| 페이지 1 마지막 칸 | `chars[579]` |
| 페이지 2 첫 칸 | `chars[580]` |
| `hp:tbl@id` | 단일 표 유지 — 페이지 수와 무관하게 동일 id |
| `rowCnt` | `29 × 페이지수` |

#### 한글 수동 열기 테스트 — 완료 (2026-06-16, 대각선 템플릿)

1. `pnpm test` 통과 확인. ✅
2. 앱에서 581자·1161자 샘플 텍스트로 `.hwpx` 다운로드. ✅
3. 한글에서 파일 열기 — 오류 대화상자 없이 열림. ✅
4. 2페이지·3페이지 출력 각각 20×29 연습장 페이지 경계 정상 표시. ✅
5. 페이지 경계·글자 위치 매핑 육안 확인. ✅
6. 대각선 가이드 선이 페이지마다 유지됨. ✅
...
**구현·검증 완료:** 대각선 템플릿 기준 다중 페이지 PoC (단일 `hp:tbl` 행 확장), 580칸 제한 제거, 한글 수동 열기 검증.

**구현하지 않은 항목:** 템플릿 선택 UI, 방안선/십자선 다중 페이지 (별도 fixture 검증 전까지 범위 밖).
...
- [x] `pnpm test` 통과
- [x] 581·1160·1161칸 자동 테스트 통과
- [x] 한글에서 2~3페이지 샘플 수동 열기 성공 (대각선 템플릿)
- [x] 단일 페이지(≤580칸) 기존 동작 회귀 없음
