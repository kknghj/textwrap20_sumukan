...
| **MVP 용량** | 단일 페이지 **20×29 = 580칸**까지만 지원. 초과 시 다운로드 차단 + 안내 문구. |
| **HWPX 모드 글자 수** | **20자/줄 고정**. 사용자 설정과 무관하게 20×29 템플릿 기준으로 생성. |
| **다중 페이지** | MVP 범위 밖. 후속 버전(로드맵 7단계). |
| **다중 템플릿** | MVP 범위 밖. 후속 버전(로드맵 8단계)에서 16·19·20칸 템플릿 검토. |
...
- **580칸(20×29) 이하만 지원.**
- 초과 시 다운로드 차단 + 안내 메시지 (Excel 등 대체 수단 언급 없음):
...
| **7** | 다중 페이지 지원 | tbl 복제, page break, 580칸 초과 허용 |
| **8** | 다중 템플릿 지원 | **16칸·19칸·20칸** 템플릿 선택 UI, 템플릿별 `maxCharsPerLine` 연동 |

#### 8단계 다중 템플릿 후보

| 템플릿 | 열 수 | 비고 |
| --- | --- | --- |
| (후보) 16칸 | 16 | 별도 HWPX 템플릿 에셋 필요 |
| (후보) 19칸 | 19 | 별도 HWPX 템플릿 에셋 필요 |
| (현재) 20칸 대각선 | 20 | `practice_diagonal_20x29` — MVP 기본 |

MVP에서는 20칸 템플릿만 사용한다.
...
| 16 | 다중 템플릿 UX | 8단계에서 16·19·20칸 지원 |
...
- **다중 템플릿** (16·19·20칸 선택) — 후속 로드맵 8단계에만 기록
- `linesegarray` 수정
